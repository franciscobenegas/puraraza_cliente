...
import { initialValues, validationSchema } from "./MotivoPesajeAbm.form";
...
export const MotivoPesajeAbm = (props) => {
    ...
    onSubmit: async (formValue) => {
      if (mode === "ADD") {
        try {
          ...
          await ApiMotivoPesajeCtrl.postData(body);
          formik.handleReset();
          setReload(true);
          setOpen(false);
        } catch (error) {
      ...
      if (mode === "DLT") {
        try {
          await ApiMotivoPesajeCtrl.delete(codId);
          formik.handleReset();
          setReload(true);
          setOpen(false);
        } catch (error) {
      ...
      if (mode === "UPD") {
        try {
          ...
          await ApiMotivoPesajeCtrl.update(body, codId);
          setReload(true);
          setOpen(false);
        } catch (error) {
          console.error(error);
