...
import { initialValues, validationSchema } from "./MotivoEntradaAbm.form";
import { useFormik } from "formik";
import { ApiMotivoEntrada } from "../../../api";
import { useAuth } from "../../../hooks";

const ApiMotivoEntradaCtrl = new ApiMotivoEntrada();

export const MotivoEntradaAbm = (props) => {
    ...
    onSubmit: async (formValue) => {
      if (mode === "ADD") {
        try {
          ...
          await ApiMotivoEntradaCtrl.postData(body);
          formik.handleReset();
          setReload(true);
          setOpen(false);
        } catch (error) {
      ...
      if (mode === "DLT") {
        try {
          await ApiMotivoEntradaCtrl.delete(codId);
          formik.handleReset();
          setReload(true);
          setOpen(false);
        } catch (error) {
      ...
      if (mode === "UPD") {
        try {
          ...
          await ApiMotivoEntradaCtrl.update(body, codId);
          setReload(true);
          setOpen(false);
        } catch (error) {
          console.error(error);
